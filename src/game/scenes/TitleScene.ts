...
import { LegacyStateAdapter } from '../legacy/LegacyStateAdapter';
...
export class TitleScene extends Phaser.Scene {
  ...
  private drawCover(): void {
    ...
    this.add.text(GAME_WIDTH / 2, 184, 'Phaser-native vertical slice • cerita dan enam ending tetap tersimpan', {
  ...
  private createMenu(): void {
    this.items = [
      ...
      { label: 'KISAH LENGKAP / ERA LEGACY', action: () => location.assign('/legacy.html') },
    ];
    ...
    this.buttons = this.items.map((item, index) => {
      const button = this.add.text(GAME_WIDTH / 2, 308 + index * 48, item.label, {
        ...
        fontSize: index === 3 ? '12px' : '15px', align: 'center', fixedWidth: 330,
        padding: { x: 16, y: 10 },
  ...
  private continueGame(): void {
    ...
    if (!saved) return;
    if (saved.era !== '1944') {
      new LegacyStateAdapter(this.save).enter(saved.era, saved.S);
      return;
    }
  ...
  private newCycle(): void {
    if (!this.save.data.game) {
      this.start1944(this.save.beginCycle());
      return;
    }
    ...
    const yes = this.confirmButton(GAME_WIDTH / 2 - 100, GAME_HEIGHT / 2 + 38, 'YA, MULAI BARU', () => {
      this.confirmPanel?.destroy(); this.confirmPanel = undefined; this.start1944(this.save.beginCycle());
    });
    const no = this.confirmButton(GAME_WIDTH / 2 + 120, GAME_HEIGHT / 2 + 38, 'BATAL', () => {
      this.confirmPanel?.destroy(); this.confirmPanel = undefined;
    });
