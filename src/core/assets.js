/* ============================================================
   ...
   ============================================================ */
const PAL={hairE:'#D2C49E',hairEs:'#ACA07E',skin:'#F0E4D8',skinS:'#E2D0BE',
...
const ASSET_MANIFEST={
  ...
  bg2088_fg:{src:'assets/bg2088_fg.png',h:150},bg1944_fg:{src:'assets/bg1944_fg.png',h:150},
...
(function loadAssets(){ // mulai dimuat saat halaman dibuka; layar 'load' menunggu
  ...
  [['assets/fonts/patrick-hand.woff2','U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+0304,U+0308,U+0329,U+2000-206F,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD'],
   ...
   .forEach(([src,ur])=>{try{const f=new FontFace('Patrick Hand',`url('${src}')`,{unicodeRange:ur});
     f.load().then(ff=>document.fonts.add(ff)).catch(()=>{}).finally(end);}catch(e){end();}});
})();
AU.bufs={};
function loadAudioBufs(){ // unduh+dekode SFX/ambience eksternal sekali (butuh konteks audio aktif)
  Object.entries(AUDIO_MANIFEST).forEach(([id,src])=>{
    fetch(src).then(r=>{if(!r.ok)throw 0;return r.arrayBuffer();})
      ...
      .then(buf=>{AU.bufs[id]=buf;
        const e=(AMB_LAYER[ambSet]||[]).find(x=>x[0]===id);if(e&&!AU.muted)ambBufLoop(id,e[1]);}) // pasang loop yg era-nya sedang aktif
      .catch(()=>{});});
}
...
function drawCharSheet(c,id,expr,phase,moving,opt={}){ // false => pemanggil pakai prosedural
  ...
  if(moving&&cols>1){const seq=cols>3?[1,2,3,2]:null,sub=phase/(Math.PI/2),i0=Math.floor(sub),a=sub-i0;
    const pick=k=>seq?seq[k%4]:k%cols;col=pick(i0);
    blend=clamp((a-.7)/.3,0,1);if(blend>0)col2=pick(i0+1);} // siklus w1→w2→w3→w2 dgn cross-fade pada 30% akhir tiap substep (anti pop)
